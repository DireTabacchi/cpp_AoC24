...
std::vector<Instruction*> Parser::parse()
{
    ...
    while (pos < src.size()) {
        if (is_letter(current)) {
            ...
            if (enabled && tok == TOKENTABLE[TokenType::mul]) {
            ...
            tok = src.substr(pos, TOKENTABLE[TokenType::do_ins].size());
            if (!enabled && tok == TOKENTABLE[TokenType::do_ins]) {
                this->next(TOKENTABLE[TokenType::do_ins].size());
                ...
                enabled = true;
                instructions.push_back(new DoIns());
                continue;
            }
            tok = src.substr(pos, TOKENTABLE[TokenType::dont_ins].size());
            if (enabled && tok == TOKENTABLE[TokenType::dont_ins]) {
                this->next(TOKENTABLE[TokenType::dont_ins].size());
                ...
                enabled = false;
                instructions.push_back(new DontIns());
                continue;
            }
...
void DoIns::print() const
{
    std::cout << "do()\n";
}

void DontIns::print() const {
    std::cout << "don't()\n";
}
